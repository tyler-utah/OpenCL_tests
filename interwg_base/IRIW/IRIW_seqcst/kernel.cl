__kernel void litmus_test(
  ...
  if (TEST_THREAD(0,0) || TEST_THREAD(0,1) || TESTING_WARP(0,0) || TESTING_WARP(0,1) ||
    ...
    if (TEST_THREAD(0,0)) {
      ...
      int tmp1 = atomic_load_explicit(&ga[y_loc], memory_order_seq_cst, memory_scope_device);
      int tmp2 = atomic_load_explicit(&ga[x_loc], memory_order_seq_cst, memory_scope_device);
      out[0] = tmp1;
      out[1] = tmp2;
    }
    else if (TEST_THREAD(0,1)) {
      test_barrier(&(bar[0]),4);
      int tmp3 = atomic_load_explicit(&ga[x_loc], memory_order_seq_cst, memory_scope_device);
      int tmp4 = atomic_load_explicit(&ga[y_loc], memory_order_seq_cst, memory_scope_device);
      out[2] = tmp3;
      out[3] = tmp4;
