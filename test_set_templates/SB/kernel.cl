__kernel void litmus_test(
  ...
  if (TEST_THREAD(0,0) || TEST_THREAD(0,1) || TESTING_WARP(0,0) || TESTING_WARP(0,1)) {
    ...
    if (TEST_THREAD(0,0)) {
      ...
      atomic_store_explicit(&ga[x_loc], 1, MEMORY_ORDER0, memory_scope_device);
      int tmp1 = atomic_load_explicit(&ga[y_loc], MEMORY_ORDER1, memory_scope_device);
      out[0] = tmp1;
    } else if (TEST_THREAD(0,1)) {
      // Work-item 0 in workgroup 1:
      test_barrier(&(bar[0]), 2);
      //atomic_fetch_add(&out[1], 1);
      atomic_store_explicit(&ga[y_loc], 1, MEMORY_ORDER2, memory_scope_device);
      int tmp2 = atomic_load_explicit(&ga[x_loc], MEMORY_ORDER3, memory_scope_device);
      out[1] = tmp2;
    }
  }
  else if (MEM_STRESS) {
